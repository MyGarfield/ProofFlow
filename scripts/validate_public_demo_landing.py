...
def _validate_evidence(repository_root: Path, site_root: Path) -> list[str]:
    ...
    runtime = evidence.get("runtime_boundary", {})
    expected_runtime = {
    ...
    flow = [item.get("step") for item in evidence.get("reference_flow", [])]
    if flow != list(FLOW):
        errors.append(f"evidence flow must be exactly {' -> '.join(FLOW)}")
    suite = evidence.get("contract_suite", {})
    if suite.get("result") != "11/11":
        errors.append("contract suite result must be the reviewed 11/11 baseline")
    if suite.get("report_hash") != (
        "sha256:f81883cca94268e35375e8fcb7eb6afb60b769e15040130cbe7359c6bb23bc17"
    ):
        errors.append("contract suite report hash drifted from the clean baseline observation")
    if suite.get("legal_accuracy_measured") is not False:
        errors.append("11/11 must not claim legal accuracy measurement")
    if suite.get("performance_measured") is not False:
        errors.append("11/11 must not claim performance measurement")
    if suite.get("official_score") is not None:
        errors.append("11/11 must not be converted into an official score")

    pins = evidence.get("input_pins", {})
    ...
    artifacts = evidence.get("public_artifacts", [])
    artifacts_by_path = {item.get("path"): item for item in artifacts if isinstance(item, dict)}
    ...
    evaluation = evidence.get("evaluation_boundary", {})
    if evaluation != {
...
def _validate_media(site_root: Path, parser: LandingHTMLParser) -> list[str]:
    ...
    if (site_root / str(video.get("path", ""))).exists():
        errors.append("an MP4 exists while the media contract still says NOT_PUBLISHED")

    subtitle_path = site_root / str(subtitles.get("path", ""))
    try:
        subtitle_bytes = subtitle_path.read_bytes()
    except OSError:
        subtitle_bytes = b""
        errors.append("declared subtitle file is missing")
    if subtitles.get("present") is not True or subtitles.get("language") != "zh-CN":
        errors.append("subtitle contract must declare the present zh-CN track")
    if subtitles.get("cue_count") != 6:
        errors.append("subtitle contract must declare exactly 6 cues")
    ...
    if not isinstance(publication_gate, list) or len(publication_gate) != 6:
        errors.append("video publication gate must keep all 6 required checks")
    return errors
    ...
    for label, content in loaded_text_assets.items():
        for pattern in FORBIDDEN_VISIBLE_CLAIMS:
            if pattern.search(content):
                errors.append(f"forbidden claim token in loaded asset {label}: {pattern.pattern}")
